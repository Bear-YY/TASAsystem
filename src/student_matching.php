...
//カテゴリー別のGPAを計算する。
$sql = <<<EOM
SELECT sub_id ,category_id, category_name ,SUM(grade*sub_unit)/SUM(sub_unit) AS cat_gpa FROM tb_student NATURAL JOIN tb_course NATURAL JOIN tb_subject NATURAL JOIN tb_category cat WHERE stu_id = '{$stu_id}' GROUP BY cat.category_id ORDER BY cat_gpa DESC
EOM;
$rs = $conn->query($sql);
$row = $rs->fetch_assoc();
$categorygpa = [];
